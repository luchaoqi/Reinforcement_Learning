...
class RecurrentPolicy(nn.Module):
    def __init__(self,
        ...
        self._cnn = nn.Sequential(
            ...
            nn.BatchNorm2d(32),
            nn.ReLU(),
            _init_weights(nn.Conv2d(32, 64, 3, stride=2, padding=1)),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            _init_weights(nn.Conv2d(64, 128, 3, stride=2, padding=1)),
            nn.BatchNorm2d(128),
            nn.ReLU(),
            _init_weights(nn.Conv2d(128, 256, 3, stride=2, padding=1)),
            nn.BatchNorm2d(256),
            nn.ReLU()
        )
    ...
    def forward(self, input_states, masks, prev_actions, rnn_hxs):
        ...
        return self._dropout(self._critic_linear(x)), self._dropout(x), self._dropout(rnn_hxs)
